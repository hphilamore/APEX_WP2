...
def format_result(best_config, mfc_types_regex_mappings):
    """Format a model result for printing and Excel."""

    return {
        "Model": best_config["model"],
        "Window Length (h)": round(best_config["window_length"], 3),
        "N Samples": best_config["n_samples"],
        "MFC Types": ", ".join(
            mfc_types_regex_mappings[p]
            for p in best_config["mfc_types"]
        ),
        "Resistances (kOhm)": ", ".join(
            map(str, best_config["resistances"])
        ),
        "Years": ", ".join(
            map(str, best_config["years"])
        ),
        "R²": round(best_config["r2"], 3),
        "MAE": round(best_config["mae"], 3),
        "Model Parameters": ", ".join(
            f"{k}: {v:.3f}"
            for k, v in best_config["parameters"].items()
        ),
        "Terms": ", ".join(best_config["features"]),
        "Coefficients": ", ".join(
            map(str, best_config.get("coefficients", []))
        ),
        "Intercept": best_config.get("intercept", ""),
        "Feature Importances": ", ".join(
            map(str, best_config.get("feature_importances", []))
        ),
    }
...
def main():
    ...
    for features in feature_sets:
        ...
        # for window_length in [18]:
            ...
            # Store this result for each model class investigated
    ...
    # Store and visualise the results for each model, across window sizes tested 
    for results, model in zip(model_results, models):
        ...
        # Format results as list of dictionaries, showing best result for each window size
        formatted_results = [format_result(best_config, mfc_types_regex_mappings) 
        ...
        for row in formatted_results:
            ...
            print(
                f"Best config for window length "
                f"{row['Window Length (h)']} hours"
            )
